...
import {
  ...
  Package, Save, Plus, X, HelpCircle, Sparkles, Loader2
} from "lucide-react";
...
export default function CompanyProfile() {
  ...
  const addProduct = () => {
    if (!data) return;
    update("products_services", [...data.products_services, { name: "", description: "", price: "" }]);
  };
  const updateProduct = (idx: number, field: string, value: string) => {
    if (!data) return;
    const updated = [...data.products_services];
    (updated[idx] as any)[field] = value;
    update("products_services", updated);
  };
  const removeProduct = (idx: number) => {
    if (!data) return;
    update("products_services", data.products_services.filter((_, i) => i !== idx));
  };

  // FAQ CRUD
  const addFaq = () => {
    if (!data) return;
    update("objections_faq", [...data.objections_faq, { question: "", answer: "" }]);
  };
  const updateFaq = (idx: number, field: string, value: string) => {
    if (!data) return;
    const updated = [...data.objections_faq];
    (updated[idx] as any)[field] = value;
    update("objections_faq", updated);
  };
  const removeFaq = (idx: number) => {
    if (!data) return;
    update("objections_faq", data.objections_faq.filter((_, i) => i !== idx));
  };
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="identity" className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <div>
                ...
                <Textarea rows={3} value={data.description} onChange={(e) => update("description", e.target.value)} placeholder="Descreva o que sua empresa faz, qual problema resolve e para quem..." />
              </div>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>A IA usará isso para recomendar soluções aos leads.</CardDescription>
                </div>
            ...
            <CardContent className="space-y-4">
              {data.products_services.length === 0 && (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">Nenhum produto cadastrado. Adicione seus produtos para a IA recomendá-los.</p>
                </div>
              )}
              {data.products_services.map((product, idx) => (
                <div key={idx} className="p-4 rounded-lg border bg-muted/20 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <Label>Nome</Label>
                      <Input value={product.name} onChange={(e) => updateProduct(idx, "name", e.target.value)} placeholder="Nome do produto/serviço" />
                    </div>
                    <div>
                      <Label>Preço / Faixa</Label>
                      <Input value={product.price || ""} onChange={(e) => updateProduct(idx, "price", e.target.value)} placeholder="Ex: R$ 497/mês" />
                    </div>
                  </div>
                  <div>
                    <Label>Descrição</Label>
                    <Textarea rows={2} value={product.description} onChange={(e) => updateProduct(idx, "description", e.target.value)} placeholder="O que entrega? Qual problema resolve?" />
                  </div>
                </div>
              ))}
            </CardContent>
        ...
        <TabsContent value="strategy" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Público, diferenciais e tom de comunicação da empresa.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <Label className="flex items-center gap-1"><Users className="h-3.5 w-3.5" /> Público-Alvo</Label>
                <Textarea rows={3} value={data.target_audience} onChange={(e) => update("target_audience", e.target.value)} placeholder="Quem é o cliente ideal? Segmento, porte, cargo do decisor, dores principais..." />
              </div>
              <div>
                <Label className="flex items-center gap-1"><Lightbulb className="h-3.5 w-3.5" /> Diferenciais</Label>
                <Textarea rows={3} value={data.differentials} onChange={(e) => update("differentials", e.target.value)} placeholder="O que diferencia sua empresa da concorrência? Cases de sucesso, números, prêmios..." />
              </div>
              <div>
                <Label className="flex items-center gap-1"><MessageSquare className="h-3.5 w-3.5" /> Tom de Voz</Label>
                <Textarea rows={2} value={data.tone_of_voice} onChange={(e) => update("tone_of_voice", e.target.value)} placeholder="Ex: Profissional mas acessível, consultivo, sem jargão técnico..." />
        ...
        <TabsContent value="sales" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Processo de Vendas</CardTitle>
              <CardDescription>Como funciona sua venda? A IA precisa saber o fluxo.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <Label className="flex items-center gap-1"><FileText className="h-3.5 w-3.5" /> Processo Comercial</Label>
                <Textarea rows={4} value={data.sales_process} onChange={(e) => update("sales_process", e.target.value)} placeholder="Descreva as etapas: prospecção → qualificação → apresentação → proposta → fechamento. Quanto tempo cada etapa leva? Quem participa?" />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-lg flex items-center gap-2"><HelpCircle className="h-5 w-5" /> Objeções & FAQ</CardTitle>
                  <CardDescription>As objeções mais comuns e como a IA deve responder.</CardDescription>
                </div>
            ...
            <CardContent className="space-y-4">
              {data.objections_faq.length === 0 && (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">Adicione objeções comuns (ex: "Está caro", "Já tenho fornecedor") e como respondê-las.</p>
                </div>
              )}
              {data.objections_faq.map((faq, idx) => (
                <div key={idx} className="p-4 rounded-lg border bg-muted/20 space-y-3">
                  ...
                  <div>
                    <Label>Objeção / Pergunta</Label>
                    <Input value={faq.question} onChange={(e) => updateFaq(idx, "question", e.target.value)} placeholder='Ex: "Está muito caro"' />
                  </div>
                  <div>
                    <Label>Como responder</Label>
                    <Textarea rows={2} value={faq.answer} onChange={(e) => updateFaq(idx, "answer", e.target.value)} placeholder="Ex: Entendo a preocupação. Nossos clientes recuperam o investimento em média em 45 dias..." />
                  </div>
                </div>
              ))}
            </CardContent>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input value={data.email || ""} onChange={(e) => update("email", e.target.value)} placeholder="contato@empresa.com" />
                </div>
                <div>
                  <Label className="flex items-center gap-1"><Phone className="h-3.5 w-3.5" /> Telefone</Label>
                  <Input value={data.phone || ""} onChange={(e) => update("phone", e.target.value)} placeholder="(11) 99999-9999" />
                </div>
                <div>
                  <Label className="flex items-center gap-1"><Instagram className="h-3.5 w-3.5" /> Instagram</Label>
                  <Input value={data.instagram || ""} onChange={(e) => update("instagram", e.target.value)} placeholder="@empresa" />
                </div>
                <div>
                  <Label className="flex items-center gap-1"><Linkedin className="h-3.5 w-3.5" /> LinkedIn</Label>
                  <Input value={data.linkedin || ""} onChange={(e) => update("linkedin", e.target.value)} placeholder="linkedin.com/company/..." />
                </div>
                <div>
                  <Label className="flex items-center gap-1"><Facebook className="h-3.5 w-3.5" /> Facebook</Label>
                  <Input value={data.facebook || ""} onChange={(e) => update("facebook", e.target.value)} placeholder="facebook.com/..." />
                </div>
              </div>
              <div>
                <Label className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5" /> Endereço</Label>
                <Input value={data.address || ""} onChange={(e) => update("address", e.target.value)} placeholder="Rua, número, cidade - estado" />
              </div>
            </CardContent>
          </Card>
        </TabsContent>
