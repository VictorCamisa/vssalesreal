    ...
    <SidebarProvider>
      <div className="flex min-h-screen w-full bg-background">
        ...
        <main className="flex-1 flex flex-col min-w-0">
          <header className="flex h-11 items-center justify-between border-b border-border/50 px-4 shrink-0 bg-card/50 backdrop-blur-sm sticky top-0 z-10">
            <div className="flex items-center gap-2.5">
              <SidebarTrigger className="text-muted-foreground hover:text-foreground h-7 w-7" />
              {meta && (
                <div className="flex items-center gap-2 text-sm">
                  <span className="text-muted-foreground/40">/</span>
                  {PageIcon && <PageIcon className="h-3.5 w-3.5 text-muted-foreground" />}
